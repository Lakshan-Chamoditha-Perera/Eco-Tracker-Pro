...
function SignIn() {
  ...
  let setUserData = () => {
    let config = {
      ...
      url: `http://localhost:3001/user/getCustomer?email=${localStorage.getItem(
        "email"
      )}`,
      headers: {},
    };
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.data.status === 200) {
              ...
              if (localStorage.getItem("role") == "client") setUserData();
  ...
  return (
    <section
      className={
        "bg-gradient-to-t from-[#EAF5F1] to-[#FFFFFF]  px-[180px] pb-[50px] h-[100vh] flex justify-around mt-[30px]"
      }
    >
      <div
        className={
          "w-[80%] rounded-[20px] shadow bg-white justify-around  flex flex-row border"
        }
      >
        {/*left*/}
        ...
        <div className={"w-[40%] p-3  "}>
          ...
          <form className={"mt-[10px] flex flex-col justify-between "}>
            ...
            <div className={"m-2 flex flex-col  justify-end"}>
              <a
                className={
                  " flex-row text-[14px] text-gray-300 tracking-normal flex hover:text-black hover:cursor-pointer  justify-end"
                }
              >
                forgot password?
              </a>
            ...
            <div className={"m-2 text-[14px] flex justify-between"}>
              ...
              <Button
                className={
                  "hidden hover:bg-green-400 bg-[#0FAF72] text-white  lg:flex"
                }
                variant="flat"
                onClick={login}
              >
